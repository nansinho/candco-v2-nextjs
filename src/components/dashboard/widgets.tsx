  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground-subtle mt-1">CA facturé</p>
      </div>
      <div>
        <p className="text-lg sm:text-xl font-semibold tracking-tight leading-none">
          {formatCurrency(data.caEncaisse)}
        </p>
        <p className="text-xs text-muted-foreground-subtle mt-1">Encaissé</p>
      </div>
      <div>
        <p className="text-lg sm:text-xl font-semibold tracking-tight leading-none">
          {data.tauxEncaissement}
          <span className="text-sm font-normal text-muted-foreground-subtle">%</span>
        </p>
        <p className="text-xs text-muted-foreground-subtle mt-1">Encaissement</p>
      </div>
    </div>
  );
}
      ...
      {items.map((item) => (
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground-subtle mt-0.5">{item.label}</p>
          </div>
        </Link>
      ))}
    </div>
  ...
  if (sessions.length === 0) {
    return (
      <div className="flex items-center gap-3 py-1">
        <Calendar className="h-4 w-4 text-muted-foreground-faint shrink-0" />
      ...
      {sessions.map((session) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground-subtle tabular-nums shrink-0">
...
export function DevisWidget({ devis }: { devis: DevisItem[] }) {
  if (devis.length === 0) {
    return (
      <div className="flex items-center gap-3 py-1">
        <FileText className="h-4 w-4 text-muted-foreground-faint shrink-0" />
        <p className="text-xs text-muted-foreground-subtle">
          Aucun devis.{" "}
    ...
    <div className="space-y-1">
      {devis.map((d) => {
        ...
        return (
          ...
          >
            <div className={`h-1.5 w-1.5 rounded-full ${dot} shrink-0`} />
            <span className="font-mono text-xs text-muted-foreground-faint shrink-0">
              {d.numero_affichage}
            </span>
            ...
            {d.total_ttc ? (
              <span className="text-xs text-muted-foreground-subtle font-medium tabular-nums shrink-0">
...
export function FacturesWidget({ factures }: { factures: FactureItem[] }) {
  if (factures.length === 0) {
    return (
      <div className="flex items-center gap-3 py-1">
        <Receipt className="h-4 w-4 text-muted-foreground-faint shrink-0" />
        <p className="text-xs text-muted-foreground-subtle">
          Aucune facture.{" "}
...
export function SessionsActivesWidget({ data }: { data: StatsData }) {
  return (
    <div className="flex items-center gap-4">
      ...
      <Link
        href="/sessions"
        className="flex items-center gap-1.5 rounded-lg border border-border/40 px-3 py-1.5 text-xs text-muted-foreground-subtle hover:text-primary hover:border-primary/30 transition-colors"
      >
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {links.map((link) => (
        ...
        >
          <link.icon className="h-3.5 w-3.5 text-muted-foreground-faint group-hover:text-primary transition-colors" />
