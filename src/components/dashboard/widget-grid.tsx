  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/30">
        <button
          className="cursor-grab active:cursor-grabbing touch-none text-muted-foreground/60 hover:text-muted-foreground transition-colors"
          {...attributes}
          {...listeners}
        >
          <GripVertical className="h-3.5 w-3.5" />
        </button>
        <span className="text-xs font-medium text-muted-foreground flex-1 select-none">
          {title}
        </span>
        <button
          onClick={() => onToggle(id)}
          className="opacity-0 group-hover:opacity-100 text-muted-foreground/60 hover:text-muted-foreground transition-all"
        >
