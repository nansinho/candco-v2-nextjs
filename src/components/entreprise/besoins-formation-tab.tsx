          ...
          <div className="rounded-lg border border-dashed border-border/60 bg-muted/10 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Wallet className="h-4 w-4 text-muted-foreground-subtle" />
    ...
    <div className="rounded-lg border border-border/60 bg-card p-4">
          ...
          {editing ? (
            ...
            <div>
              ...
              >
                ...
                <Pencil className="h-3 w-3 text-muted-foreground-subtle group-hover:text-primary transition-colors" />
              </button>
              <p className="text-xs text-muted-foreground-subtle mt-0.5">Cliquer pour modifier</p>
            </div>
          )}
        </div>
      ...
      {budget.budgetTotal > 0 && (
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground-subtle text-right">{pct}% engagé</p>
        </div>
      )}
    </div>
  );
        ...
        <div className="space-y-1.5">
          ...
          {selectedProduit ? (
            <div className="flex items-center gap-2 rounded-md border border-primary/30 bg-primary/5 px-3 py-2">
              ...
              <span className="text-xs font-mono text-muted-foreground-subtle">{selectedProduit.numero_affichage}</span>
              {selectedTarif && (
                  ...
                  <div className="max-h-48 overflow-auto">
                    {searchLoading ? (
                      ...
                      produitResults.map((p) => (
                        ...
                        >
                          <BookOpen className="h-3 w-3 text-muted-foreground-subtle shrink-0" />
                          <span className="text-xs truncate">{p.intitule}</span>
                          <span className="text-xs font-mono text-muted-foreground-subtle shrink-0">{p.numero_affichage}</span>
                        </button>
                      ))
                    )}
                  </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs font-medium truncate">
                        {tarif.nom || "Tarif"}
                        {tarif.is_default && (
                          <span className="ml-1 text-xs text-muted-foreground-subtle">(défaut)</span>
                        )}
                      </span>
                      <div className="ml-auto flex items-center gap-2 shrink-0 text-xs">
                        ...
                        {tarif.unite && (
                          <span className="text-muted-foreground-subtle">/ {tarif.unite}</span>
                        )}
                      </div>
                    </button>
                  );
    ...
    <div className="rounded-lg border border-border/60 bg-card px-4 py-3 space-y-2 group">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <GraduationCap className="h-4 w-4 text-muted-foreground-subtle shrink-0" />
      ...
      {b.sessions ? (
        ...
        <div className="flex items-center gap-2">
          <Link2 className="h-3 w-3 text-muted-foreground-faint" />
          <span className="text-xs text-muted-foreground-subtle">Pas de session liée</span>
        </div>
      ) : null}
    </div>
  );
