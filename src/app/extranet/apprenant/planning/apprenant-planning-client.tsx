...
import { getApprenantCreneaux, type ApprenantPlanningCreneau } from "@/actions/extranet-context";
import {
  getWeekDays,
  getWeekRange,
  formatDayHeader,
...
function ApprenantWeekEvent({
  creneau,
  style,
  compact = false,
  onClick,
}: {
  creneau: ApprenantPlanningCreneau;
        ...
        <div className="flex flex-col gap-0.5 min-w-0 h-full">
          ...
          {creneau.formateur && (
            <div className="flex items-center gap-1 min-w-0 mt-auto">
              <User className="h-2.5 w-2.5 text-muted-foreground-subtle shrink-0" />
  ...
  return (
    <div className="w-72 rounded-xl border border-border bg-popover shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
      ...
      <div className="px-4 py-3 space-y-2.5">
        ...
        {creneau.formateur && (
          <div className="flex items-center gap-2">
            <User className="h-3.5 w-3.5 text-muted-foreground-subtle shrink-0" />
...
interface ApprenantPlanningClientProps {
  apprenantId: string;
}

export function ApprenantPlanningClient({ apprenantId }: ApprenantPlanningClientProps) {
  const [currentDate, setCurrentDate] = React.useState(new Date());
  const [loading, setLoading] = React.useState(true);
  const [creneaux, setCreneaux] = React.useState<ApprenantPlanningCreneau[]>([]);
  const [selectedCreneau, setSelectedCreneau] = React.useState<ApprenantPlanningCreneau | null>(null);
  ...
  const creneauxByDay = React.useMemo(() => {
    const map = new Map<string, ApprenantPlanningCreneau[]>();
              ...
              {selectedCreneau && popoverPos && (
                <>
                  ...
                  <div className="absolute z-40" style={{ left: popoverPos.x, top: popoverPos.y }}>
                    <ApprenantEventDetail creneau={selectedCreneau} onClose={closePopover} />
                  </div>
                </>
              )}
