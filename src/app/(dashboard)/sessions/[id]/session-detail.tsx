            ...
            <div className="rounded-lg border border-border/60 bg-card">
                    ...
                    {linkedDevis.map((d) => (
                      <div key={d.id} className="flex items-center justify-between rounded-md border border-border/40 bg-muted/20 px-3 py-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <FileText className="h-3.5 w-3.5 text-muted-foreground-subtle shrink-0" />
              ...
              <div className="flex items-center justify-between border-t border-border/60 px-6 py-4">
                <div className="flex items-center gap-4">
                  <p className="text-xs text-muted-foreground-subtle">Créé le {formatDate(session.created_at)}</p>
                  {session.updated_at && <p className="text-xs text-muted-foreground-subtle">Modifié le {formatDate(session.updated_at)}</p>}
                </div>
          ...
          <div className="space-y-4">
            ...
            {commanditaires.length === 0 && !showAddCommanditaire ? (
              <div className="flex flex-col items-center gap-3 rounded-lg border border-border/60 bg-card py-16">
                <Building2 className="h-8 w-8 text-muted-foreground-faint" />
        ...
        <TabsContent value="apprenants" className="mt-6">
          <div className="space-y-4">
            ...
            {inscriptions.length === 0 && !showAddApprenant ? (
              <div className="flex flex-col items-center gap-3 rounded-lg border border-border/60 bg-card py-16">
                <UserPlus className="h-8 w-8 text-muted-foreground-faint" />
                      ...
                      {inscriptions.map((insc) => (
                        <tr key={insc.id} className="border-b border-border/40 hover:bg-muted/20 group">
                          <td className="px-4 py-2.5 text-sm font-medium">
                            ...
                            <span className="ml-2 text-xs font-mono text-muted-foreground-subtle">{insc.apprenants?.numero_affichage}</span>
                          </td>
                ...
                {creneaux.map((c) => (
                  <div key={c.id} className="flex flex-wrap items-center gap-3 sm:gap-4 rounded-lg border border-border/60 bg-card px-4 py-3 group">
                    <div className="flex items-center gap-2 min-w-[130px]">
                      <CalendarDays className="h-3.5 w-3.5 text-muted-foreground-subtle" />
                      <span className="text-sm font-medium">{formatDate(c.date)}</span>
                    </div>
                    <div className="flex items-center gap-1.5 min-w-[120px]">
                      <Clock className="h-3 w-3 text-muted-foreground-subtle" />
        ...
        <TabsContent value="emargement" className="mt-6">
          <div className="space-y-4">
            ...
            {emargements.length === 0 ? (
              <div className="flex flex-col items-center gap-3 rounded-lg border border-border/60 bg-card py-16">
                <ClipboardCheck className="h-8 w-8 text-muted-foreground-faint" />
                <p className="text-sm text-muted-foreground/60">Aucun créneau planifié</p>
              </div>
            ) : (
              <div className="space-y-4">
                            ...
                            return (
                              <div key={insc.apprenant_id} className="flex items-center justify-between px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-mono text-muted-foreground-subtle">{insc.apprenants?.numero_affichage}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  <Button
                                  ...
                                  {emargement?.heure_signature && (
                                    <span className="text-xs text-muted-foreground-subtle ml-2">
              ...
              <div className="rounded-md border border-border/60 bg-card overflow-hidden">
                {filteredApprenants.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-8">
                    <Users className="h-6 w-6 text-muted-foreground-faint" />
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-[50px_1fr_1fr_1fr_1fr] gap-1 px-3 py-1.5 border-b border-border/60 bg-muted/30">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground-subtle">Civ.</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground-subtle">Nom</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground-subtle">Prénom</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground-subtle">Email</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground-subtle">Agence</span>
                    </div>
                    {/* Rows */}
  ...
  return (
    <div className="space-y-4">
      ...
      {documents.length === 0 && !showUpload ? (
        <div className="flex flex-col items-center gap-3 rounded-lg border border-border/60 bg-card py-16">
          <FileText className="h-8 w-8 text-muted-foreground-faint" />
    ...
    <div className="space-y-4">
      ...
      {evaluations.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/60 py-12">
          <ClipboardList className="h-8 w-8 text-muted-foreground-faint" />
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-card overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-3 min-w-0">
          <ClipboardList className="h-4 w-4 text-muted-foreground-subtle shrink-0" />
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              ...
              {planification?.herite_du_produit && !planification.personnalise && (
                <span className="text-xs text-muted-foreground-subtle">Hérité du programme</span>
              )}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          {planification && (
            ...
            >
              {toggling ? (
                ...
                <ToggleLeft className="h-3.5 w-3.5 text-muted-foreground-subtle" />
              )}
            </Button>
          )}
          {planification && (
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-muted-foreground-subtle hover:text-destructive"
            onClick={onRemove}
            title="Retirer"
          >
