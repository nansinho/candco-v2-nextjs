...
function ContactsTab({ entrepriseId }: { entrepriseId: string }) {
  ...
  const handleLink = async (contactId: string) => {
    ...
    setSearchQuery(""); setSearchResults([]); setShowSearch(false); fetchContacts();
  ...
  const handleCreateContact = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setShowCreate(false); setIsCreating(false); setNewContactFonction(""); fetchContacts();
    ...
    <section className="rounded-lg border border-border/60 bg-card">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border/60">
        ...
        <div className="flex gap-1.5">
          <Button variant="outline" size="sm" className="h-7 text-[11px] border-border/60" onClick={() => { setShowCreate(!showCreate); setShowSearch(false); }}>
            <UserPlus className="mr-1 h-3 w-3" />Créer un contact
          </Button>
          <Button variant="outline" size="sm" className="h-7 text-[11px] border-border/60" onClick={() => { setShowSearch(!showSearch); setShowCreate(false); }}>
            {showSearch ? <><X className="mr-1 h-3 w-3" />Fermer</> : <><Plus className="mr-1 h-3 w-3" />Rattacher existant</>}
          </Button>
        </div>
      </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreateContact} className="px-5 py-4 border-b border-border/40 bg-muted/10">
          ...
          <div className="flex justify-end gap-2 mt-4">
            <Button type="button" variant="outline" size="sm" className="h-7 text-[11px]" onClick={() => setShowCreate(false)}>Annuler</Button>
            ...
            <div className="mt-2 space-y-1">
              {searchResults.map((c) => (
                <div key={c.id} className="flex items-center justify-between rounded-md px-3 py-2 hover:bg-muted/30 transition-colors">
                  ...
                  <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => handleLink(c.id)}><Plus className="mr-0.5 h-3 w-3" />Rattacher</Button>
                </div>
              ))}
            </div>
          )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-16">
          ...
          <div className="text-center">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground/40">Créez un nouveau contact ou rattachez un contact existant.</p>
          </div>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {contacts.map((c) => (
                <tr key={c.id} className="border-b border-border/40 transition-colors hover:bg-muted/20 group cursor-pointer" onClick={() => router.push(`/contacts-clients/${c.id}`)}>
                  <td className="px-4 py-2.5"><span className="font-mono text-xs text-muted-foreground">{c.numero_affichage}</span></td>
                  <td className="px-4 py-2.5"><div className="flex items-center gap-2"><Users className="h-3.5 w-3.5 text-purple-400" /><span className="text-[13px] font-medium">{c.prenom} {c.nom}</span></div></td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => { e.stopPropagation(); router.push(`/contacts-clients/${c.id}`); }} className="p-1 rounded hover:bg-muted/30 text-muted-foreground/40 hover:text-foreground" title="Modifier le contact"><ExternalLink className="h-3.5 w-3.5" /></button>
                      <button onClick={(e) => { e.stopPropagation(); handleUnlink(c.id); }} className="p-1 rounded hover:bg-destructive/10 text-muted-foreground/40 hover:text-destructive" title="Retirer de l'entreprise"><Unlink className="h-3.5 w-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
